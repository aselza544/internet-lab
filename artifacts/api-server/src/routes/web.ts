...
function forwardedRequestHeaders(req: Request): Record<string, string> { const headers: Record<string, string> = {}; for (const name of ["accept", "accept-language", "content-type", "range"]) { const value = req.header(name); if (value) headers[name] = value; } return headers; }
...
function verifyResourceCap(token: string, sessionId: string, targetUrl: string, plan: GatewayPlan): boolean {
  ...
  try { const cap = JSON.parse(Buffer.from(payload, "base64url").toString()) as ResourceCap; return cap.sessionId === sessionId && cap.targetUrl === targetUrl && cap.plan === plan && Number.isFinite(cap.expiresAt) && cap.expiresAt > now(); } catch { return false; }
}
...
async function serve(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (type === "text/html" || type === "application/xhtml+xml") body = Buffer.from(rewriteHtml(body.toString("utf8"), base, proxyOrigin, session.sessionId, session.plan)); else if (type === "text/css") body = Buffer.from(rewriteCss(body.toString("utf8"), base, session.sessionId, session.plan));
...
function cspHeader(proxyOrigin: string): string { return `default-src 'none'; style-src ${proxyOrigin} 'unsafe-inline'; img-src ${proxyOrigin} data: blob:; font-src ${proxyOrigin} data:; script-src ${proxyOrigin} 'unsafe-inline'; connect-src 'none'; media-src ${proxyOrigin} blob:; frame-src ${proxyOrigin}; child-src ${proxyOrigin}; worker-src ${proxyOrigin} blob:; object-src 'none'; base-uri 'none'; form-action ${proxyOrigin}; frame-ancestors 'none';`; }
